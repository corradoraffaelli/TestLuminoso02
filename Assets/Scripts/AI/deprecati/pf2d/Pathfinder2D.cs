...
public class Pathfinder2D : MonoBehaviour 
{
    ...
    {
        startNode = FindClosestNode(start, true, false);
        endNode = FindClosestNode(end, false, true);
    }

	private Node FindClosestNode(Vector3 pos, bool approsimateStartNode=false, bool approsimateEndNode=false )
	{
		Node n = null;
		...
		try {
			if(x<limitX && y<limitY)
				n = Map[x, y];
			//else 
				//UnityEngine.Debug.Log("fuori dai limiti111111111111111111");
			...
			UnityEngine.Debug.Log ("FINDER2D : non riuscito alla prima");
		}
		...
		//ha problemi se passo sui muri laterali!! -> non è un vero problema, in quanto non ci dovrei poter arrivare...


		//APPROSSIMA START
		if(approsimateStartNode) {
			
			for(int t=1; t<6;t++) {
				//UnityEngine.Debug.Log("START APPR" + t);
				int xx = (MapStartPosition.x < 0F) ? Mathf.FloorToInt(((pos.x + Mathf.Abs(MapStartPosition.x)) / Tilesize)) : Mathf.FloorToInt((pos.x - MapStartPosition.x) / Tilesize);
				int yy = (MapStartPosition.y < 0F) ? Mathf.FloorToInt(((pos.y - 3*t + Mathf.Abs(MapStartPosition.y)) / Tilesize)) : Mathf.FloorToInt((pos.y - 3*t - MapStartPosition.y) / Tilesize);

				try {
					if(xx<limitX && yy<limitY)
						n = Map[xx, yy];
					//else 
						//UnityEngine.Debug.Log("fuori dai limiti2222222222222222222222222 " + t);
					
					if(n!=null) {
						if (n.walkable)
						{
							return new Node(xx, yy, n.yCoord, n.ID, n.xCoord, n.zCoord, n.walkable);
						}
					}
				}
				catch(Exception e) {
					UnityEngine.Debug.Log ("FINDER2D : non riuscito alla seconda start");
				}
			}
		...
		//APPROSSIMA END
		if(approsimateEndNode) {
			
			for(int t=1; t<6;t++) {
				//UnityEngine.Debug.Log("END APPR" + t);

				for(int z=1; z<6; z++) {

					//GUARDO A SINISTRA
					int xx = (MapStartPosition.x < 0F) ? Mathf.FloorToInt(((pos.x - 2*z + Mathf.Abs(MapStartPosition.x)) / Tilesize)) : Mathf.FloorToInt((pos.x - 2*z - MapStartPosition.x) / Tilesize);
					int yy = (MapStartPosition.y < 0F) ? Mathf.FloorToInt(((pos.y - 3*t + Mathf.Abs(MapStartPosition.y)) / Tilesize)) : Mathf.FloorToInt((pos.y - 3*t - MapStartPosition.y) / Tilesize);
					...
					try {
						...
						UnityEngine.Debug.Log ("FINDER2D : non riuscito alla seconda end sx");
					}
					//else 
						//UnityEngine.Debug.Log("fuori dai limiti333333333333333333333333 " + t);
					


					//GUARDO A DESTRA
					xx = (MapStartPosition.x < 0F) ? Mathf.FloorToInt(((pos.x + 2*z + Mathf.Abs(MapStartPosition.x)) / Tilesize)) : Mathf.FloorToInt((pos.x + 2*z - MapStartPosition.x) / Tilesize);
					yy = (MapStartPosition.y < 0F) ? Mathf.FloorToInt(((pos.y - 3*t + Mathf.Abs(MapStartPosition.y)) / Tilesize)) : Mathf.FloorToInt((pos.y - 3*t - MapStartPosition.y) / Tilesize);
					...
					try {
						...
						UnityEngine.Debug.Log ("FINDER2D : non riuscito alla seconda end dx");
					}
